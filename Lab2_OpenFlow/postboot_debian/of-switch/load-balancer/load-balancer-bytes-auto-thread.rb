class LoadBalancer < Controller
  ...
  def switch_ready(datapath_id)
    ...
    # Create a new thread sending out flow stats request once every 5 seconds
    Thread.new do
      while 1 do
  ...
  def stats_reply (datapath_id, message)
    ...
    if(flow_count != 0)
      message.stats.each do | flow_msg |
        ...
        if(flow_msg.actions[0].port_number == @left)
            ...
            left_total_throughput += flow_msg.byte_count/flow_msg.duration_sec
            ...
            right_total_throughput += flow_msg.byte_count/flow_msg.duration_sec
    ...
    if (left_returned == 1) 
      ...
      file.puts "left #{@left_flow} #{@left_byte} #{@left_packet} #{@left_total_throughput} Bps"
    end
    if (right_returned == 1) 
      @right_flow = right_flow_count
      @right_packet = right_packet_count
      @right_byte = right_byte_count
      file.puts "right #{@right_flow} #{@right_byte} #{@right_packet} #{@right_total_throughput} Bps"
    end
    file.close
  end
  ...
  def decide_path()
    ...
    if (@left_byte > @right_byte)
        info "since there are more bytes going to the left path, let's go *RIGHT* for this one"
        path = @right
    else
        info "since there are not more bytes going to the left path, let's go *LEFT* for this one"
        path = @left
    end
    return path
  end
