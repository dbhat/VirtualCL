...
class MultiLearningSwitch < Controller
  ...
  def start
   ...
   @packet_count = {"i2" => 0, "esnet" => 0, "ornl" => 0}
    ...
    @byte_countn = {"i2" => 0, "esnet" => 0, "ornl" => 0}
    @packet_countn = {"i2" => 0, "esnet" => 0, "ornl" => 0}
    @flow_countn = {"i2" => 0, "esnet" => 0, "ornl" => 0}
    @throughputn = {"i2" => 0, "esnet" => 0, "ornl" => 0}
     @inst_throughputv = {"i2" => 0, "esnet" => 0, "ornl" => 0}
    @byte_countv = {"i2" => 0, "esnet" => 0, "ornl" => 0}
    @packet_countv = {"i2" => 0, "esnet" => 0, "ornl" => 0}
    @flow_countv = {"i2" => 0, "esnet" => 0, "ornl" => 0}
    @throughputv= {"i2" => 0, "esnet" => 0, "ornl" => 0}
    @inst_throughputv = {"i2" => 0, "esnet" => 0, "ornl" => 0}
end
...
#    Thread.new do
 ...
 # Process.detach(pid)
  ...
  def process_flow_stats (dpid, message)
    ...
    #throughputn = {"i2" => 0, "esnet" => 0, "ornl" => 0}
    @inst_throughput = {"i2" => 0, "esnet" => 0, "ornl" => 0}
    @inst_throughputn = {"i2" => 0, "esnet" => 0, "ornl" => 0}
    @inst_throughputv = {"i2" => 0, "esnet" => 0, "ornl" => 0}
    #msgnowcast = Match.new(:dl_type => 0x0800,:nw_proto => 6, :tp_src => 388)
   # msgvlc = Match.new(:dl_type => 0x0800, :nw_proto => 6, :nw_src => 192.168.10.5)
    ...
    if(total_flow_count != 0)
      message.stats.each do | flow_msg |
	    ...
	    @packet_count[path] += flow_msg.packet_count
           ...
           puts "MessageContents #{flow_msg}"
           if flow_msg.match.tp_src == 388
              puts "Nowcast"
              @flow_countn[path] = @flow_countn[path] + 1
              @byte_countn[path] += flow_msg.byte_count
              @packet_countn[path] += flow_msg.packet_count
              if flow_msg.duration_sec + flow_msg.duration_nsec/1000000000 != 0
                 @throughputn[path] += flow_msg.byte_count/(flow_msg.duration_sec + flow_msg.duration_nsec/1000000000)
                 @inst_throughputn[path] = flow_msg.byte_count/(flow_msg.duration_sec + flow_msg.duration_nsec/1000000000)
              end
           elsif flow_msg.match.tp_src == 80
              puts "VLCpacket"
              @flow_countv[path] = @flow_countv[path] + 1
              @byte_countv[path] += flow_msg.byte_count
              @packet_countv[path] += flow_msg.packet_count
              if flow_msg.duration_sec + flow_msg.duration_nsec/1000000000 != 0
                @throughputv[path] += flow_msg.byte_count/(flow_msg.duration_sec + flow_msg.duration_nsec/1000000000)
                @inst_throughputv[path] = flow_msg.byte_count/(flow_msg.duration_sec + flow_msg.duration_nsec/1000000000)
              end
           end
          end
        end
      ...
      ['esnet', 'i2','ornl'].each do | path |
       ...
       #@inst_throughput[path] = (@throughput[path] - @prev_byte_count[path])/@stats_period  
	    ...
	    file.close
            file = File.open("/tmp/flowstatsapp.out", "a")
            file.puts "#{path} #{dpid} #{@flow_countn[path].to_s} #{@byte_countn[path]} #{@packet_countn[path]} #{@throughputn[path]} Bps #{@inst_throughputn[path]} Bps Nowcast"
            file.puts "#{path} #{dpid} #{@flow_countv[path].to_s} #{@byte_countv[path]} #{@packet_countv[path]} #{@throughputv[path]} Bps #{@inst_throughputv[path]} Bps VLC"
            file.close
